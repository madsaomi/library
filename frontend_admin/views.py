...
def dashboard(request):
    ...
    month_abbrs = [_("Yan"), _("Fev"), _("Mar"), _("Apr"), _("May"), _("Iyun"), _("Iyl"), _("Avg"), _("Sen"), _("Okt"), _("Noy"), _("Dek")]
    monthly_issues = []
...
def districts_list(request):
    ...
    return render(request, 'admin_panel/districts.html', {
        'districts': districts,
...
def statistics(request):
    ...
    for i in range(period_days - 1, -1, -1):
        ...
        issue_counts.append(BookIssue.objects.filter(issued_at__date=day).count())
        return_counts.append(BookIssue.objects.filter(returned_at__date=day).count())
    ...
    issued_today = BookIssue.objects.filter(issued_at__date=today).count()
    returned_today = BookIssue.objects.filter(returned_at__date=today).count()
    active_loans = BookIssue.objects.filter(is_returned=False).count()
    total_xp = CustomUser.objects.aggregate(total=Sum('xp_points'))['total'] or 0
    
    # Most popular book
    top_book_data = BookIssue.objects.values('book__title').annotate(
        cnt=Count('id')
    ).order_by('-cnt').first()
    ...
    top_readers = CustomUser.objects.filter(
        role='student', bookissue__issued_at__date__gte=period_start
    ).annotate(
...
def user_detail(request, pk):
    ...
    all_issues = BookIssue.objects.filter(user=user)
...
def all_active_loans_list(request):
    ...
    loan_books_qs = Book.objects.filter(bookissue__is_returned=False).distinct()
    ...
    return render(request, 'admin_panel/all_active_loans.html', {
