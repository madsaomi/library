...
def check_achievements(user):
    ...
    for ach in Achievement.objects.all():
        ...
        if ach.condition_type == 'books_count':
            ...
            cats_read = BookIssue.objects.filter(
                user=user, is_returned=True, book__category__isnull=False
            ...
            speed_returns = BookIssue.objects.filter(
                user=user, is_returned=True,
                returned_at__lte=F('issued_at') + timedelta(days=3)
            ).count()
