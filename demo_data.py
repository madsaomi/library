...
from schools.models import District, GradePromotionLog, Institution, News, School, Subject
...
for dn, cnt in school_names.items():
    for i in range(cnt):
        s = School(
            name=f'{dn} {i + 1}-son maktab', address=f'{dn} tumani', contact='+998901234567', district=districts[dn]
        )
        schools.append(s)
...
for s in schools:
    u = CustomUser(
        username=f'admin{s.id}',
        password=pw,
        first_name=f'Admin{s.id}',
        last_name='Maktab',
        role='school_admin',
        school=s,
    )
...
for s in schools:
    for j in range(random.randint(3, 5)):
        ...
        u = CustomUser(
            username=f'teacher{s.id}_{j}',
            password=pw_t,
            first_name=f"O'qit.{subj[:5]}",
            last_name=f'Sch{s.id}',
            role='teacher',
            school=s,
            subject=subj,
        )
...
for s in schools:
    for j in range(random.randint(30, 55)):
        ...
        u = CustomUser(
            username=f'student{s.id}_{j}',
            password=pw_s,
            first_name=fn,
            last_name=f'Fam{s.id}_{j}',
            role='student',
            school=s,
            grade=g,
            birth_date=date(2016 - int(g) if int(g) < 11 else 2005, random.randint(1, 12), random.randint(1, 28)),
...
for school in schools:
    for grade_num in range(1, 12):
        ...
        for _ in range(num_per_grade):
            ...
            b = Book(
                school=school,
                title=f'{sn} {grade_num}-sinf',
                author=sn,
                description=f'{sn} darsligi {grade_num}-sinf',
                category=categories[random.randint(0, min(3, len(categories) - 1))],
                total_count=cnt,
                available_count=random.randint(5, cnt - 5),
                is_textbook=True,
                subject=sn,
                grade=grade_num,
            )
...
for school in schools:
    for _ in range(reg_per_school):
        ...
        b = Book(
            school=school,
            title=f'Kitob #{random.randint(1000, 9999)}',
            author=random.choice(
                ['Alisher Navoiy', 'Oybek', "G'afur G'ulom", "O'tkir Hoshimov", 'Chingiz Aytmatov', 'Jack London']
            ),
            description='Badiiy kitob',
            category=random.choice(categories),
            total_count=cnt,
            available_count=random.randint(0, cnt),
            is_textbook=False,
        )
        book_objects.append(b)
        if len(book_objects) >= BATCH:
    ...
    for tb in tbs:
        ...
        tloans.append(
            TextbookLoan(
                ...
                returned_at=date(2026, random.randint(1, 5), random.randint(1, 28)) if random.random() < 0.08 else None,
            )
        )
        if len(tloans) >= BATCH:
...
for student in students[: min(len(students), 300)]:
    for bk_id in random.sample(regular_ids, min(random.randint(0, 10), len(regular_ids))):
        ...
        issues.append(
            BookIssue(
                book_id=bk_id, user=student, issued_at=issued, returned_at=returned, is_returned=returned is not None
            )
        )
        if len(issues) >= BATCH:
...
Challenge.objects.bulk_create(
    [
        Challenge(
            title=f"O'qish marafoni {i}",
            description=f"Bu oyda {5 + i * 5} ta kitob o'qing!",
...
print('Creating news...')
news_titles = [
    "Kutubxona yangi kitoblar bilan to'ldirildi",
    ...
    for t in random.sample(news_titles, random.randint(2, 4)):
        news_list.append(
            News(
                school=s,
                title=t,
                body=f'{t}. Maktabimizdagi muhim voqea.',
                is_published=True,
