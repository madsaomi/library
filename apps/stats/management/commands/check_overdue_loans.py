...
from books.models import BookIssue, TextbookLoan
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        overdue_textbooks = TextbookLoan.objects.filter(returned_at__isnull=True, due_date__lt=today)
        tb_notified = 0
        ...
        overdue_books = BookIssue.objects.filter(is_returned=False, issued_at__date__lt=cutoff)
        bk_notified = 0
        ...
        self.stdout.write(
            self.style.SUCCESS(f'Overdue checks done: {tb_notified} textbook notices, {bk_notified} book notices sent')
        )
